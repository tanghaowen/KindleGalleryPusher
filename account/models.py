...
USER_BASE_BANDWIDTH = 1024
INVITED_USER_BASE_BANDWIDTH = 1024
...
class User(AbstractUser):
    ...
    # 每个月会重置的流量
    bandwidth_tmp = models.IntegerField(default=0, verbose_name='データ量')
    # 永久流量
    ...
    inviter = models.ForeignKey("self", null=True, verbose_name='Inviter', on_delete=models.DO_NOTHING)
        ...
        if self.vip:
            if self.have_cost_bandwidth_recently(volume):
                if action == 'download':
                    ...
                    record = DownloadRecord(user=self, volume=volume, info='No Cost', volume_type=volume_type,
                                            vip=self.vip)
                    record.save()
                ...
                if self.bandwidth_remain >= volume_bandwidth:
                    ...
                    record = DownloadRecord(user=self, volume=volume, info='cost', volume_type=volume_type,
                                            vip=self.vip)
                    record.save()
            ...
            if volume_type == VOLUME_TYPE_ZIP:
                return {'status': ONLY_VIP}
            elif volume_type == VOLUME_TYPE_MOBI:
                volume_bandwidth = volume.mobi_file.size / 1024.0 / 1024.0
            elif volume_type == VOLUME_TYPE_EPUB:
                volume_bandwidth = volume.epub_file.size / 1024.0 / 1024.0
            else:
                return {"status": ERROR}
            # 如果是特别推荐里的书籍，流量消耗为0
    ...
    def add_bandwidth_cost(self, volume):
        ...
        records = BandwidthCostRecord.objects.filter(user=self, volume=volume, cost_date__gt=time_threshold,
                                                     action='download')
        if len(records) == 0:
            volume_bandwidth = volume.get_volume_bandwidth_cost()
            if self.bandwidth_remain >= volume_bandwidth:
...
class Comment(models.Model):
    book = models.ForeignKey(Book, on_delete=models.DO_NOTHING, verbose_name='関連の写真集')
    user = models.ForeignKey(User, on_delete=models.DO_NOTHING, verbose_name='レビューしたユーザ')
    message = models.TextField(verbose_name='内容')
    comment_time = models.DateTimeField(default=now, verbose_name='レビューした時間')
    agree = models.PositiveIntegerField(default=0, verbose_name='役に立ったと思う人数')
    nouse = models.PositiveIntegerField(default=0, verbose_name='役に立たなかったと思う人数')
...
class Score(models.Model):
    score = models.PositiveIntegerField(null=False, blank=False, verbose_name="点数")
    book = models.OneToOneField(Book, verbose_name='写真集', on_delete=models.DO_NOTHING)
    user = models.OneToOneField(User, verbose_name='点数を付けたユーザ', on_delete=models.DO_NOTHING)
...
# 导致了循环import，出错，艹
# データ消費履歴、詳しく記録するため、後にもしbugがあって、ユーザが不正ダウンロードしたなら
# ここのデータを分析することによって、不正を検知できる
class BandwidthCostRecord(models.Model):
    user = models.ForeignKey(User, on_delete=models.DO_NOTHING, verbose_name='ユーザ')
    volume = models.ForeignKey(Volume, on_delete=models.DO_NOTHING, verbose_name='アルバム')
    volume_type = models.CharField(null=True, max_length=100, verbose_name='ファイルフォーマット')
    cost_date = models.DateTimeField(default=now, verbose_name='使った時間')
    # 下载消耗的流量，单位MB
    bandwidth_cost = models.IntegerField(null=False, blank=False, verbose_name='消費したデータ量')
    user_bandwidth_before = models.IntegerField(null=False, blank=False, verbose_name='消費前の残りデータ量')
    user_bandwidth_after = models.IntegerField(null=False, blank=False, verbose_name='消費後の残りデータ量')
    # 是什么类型消费了流量，分为download和push
    action = models.CharField(max_length=20, null=False, blank=False, verbose_name='消費type( download or push)')
...
class MailVertifySendRecord(models.Model):
    ...
    # 重置密码的话，关联的uid
    relative_uid = models.IntegerField(null=True, verbose_name='関連uid')
