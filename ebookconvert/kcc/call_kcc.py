...
class SlimmingEpubThread(QThread):
    ...
    def imageSlimming(self, image_infos, image_path, average_size_per_image):
        ...
        new_average_size_per_heavy_image = (epub_over_heavy_size * 1024.0 - ok_image_total_size) / (
                    total_image_count - ok_image_count)

        for img_info in image_infos:
            image_name, full_path, image_size = img_info
            print("图片: %s, %.2f/%.2f/%.2f" % (
            image_name, image_size, average_size_per_image, new_average_size_per_heavy_image))
            if image_size > average_size_per_image:
                print("开始瘦身此图片")
